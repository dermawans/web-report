...
class Model_app extends CI_Model{
	...
	// Bagian Dashboard 
		function getCountAktifProjectQT(){
			...
			return $this->db->query("select id_history from projecthistory where isactive=1 and status_project in ('In Progress','Extend','Selftest','Pending') and id_qtname='".$id_user."'
			");	
		}
		function getCountAktifProjectPMO(){
			$id_user = $this->session->userdata('ID'); 
			return $this->db->query("select id_history from projecthistory where isactive=1 and status_project in ('In Progress','Extend','Selftest','Pending') and id_pmoname='".$id_user."'
			");	
		} 
		function getCountAllAktifProject(){
			return $this->db->query("select id_history from projecthistory where isactive=1 and status_project in('In Progress','Extend','Selftest','Pending')
			");	
		} 
